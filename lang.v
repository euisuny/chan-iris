...
Inductive base_lit : Set :=
  | LitChan (c : chan) | LitInt (n : Z) | LitBool (b : bool) | LitUnit | LitProphecy (p: proph_id).
...
Definition state_upd_heap (f: gmap loc (option chan) → gmap loc (option chan)) (σ: state) : state :=
  {| heap := f σ.(heap) |}.
Global Arguments state_upd_heap _ !_ /.

Fixpoint heap_array (l : loc) (vs : list chan) : gmap loc (option chan) :=
  match vs with
  | [] => ∅
...
Lemma heap_array_map_disjoint (h : gmap loc (option chan)) (l : loc) (vs : list chan) :
...
Definition state_init_heap (l : loc) (n : Z) (v : chan) (σ : state) : state :=
...
(* TODO : Add [head_step] for channel items *)
